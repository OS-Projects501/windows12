...
SYSCALL get_bsm(int *avail) {
  ...
  for (int i = 0; i < N_BS; i++) {
    if (bsm_tab[i].bs_status == BSM_UNMAPPED) {
      *avail = i; // Assign available backing store index
...
SYSCALL free_bsm(int bs_id) {
  ...
  if (bs_id < 0 || bs_id >= N_BS) {
    ...
    //BSM_DEBUG_PRINTF("Invalid backing store ID: %d\n Choose between 0 and %d", bs_id, N_BS - 1);
    restore(ps);
    return SYSERR;
  }

  //backing store is already free -> error
  if (bsm_tab[bs_id].bs_status == BSM_UNMAPPED) {
    //BSM_DEBUG_PRINTF("Backing store %d is already free. This is an error", bs_id);
    restore(ps);
    return SYSERR;
  }
...
SYSCALL bsm_lookup(int pid, unsigned long vaddr, int *store, int *pageth) {
  ...
  for (int i = 0; i < N_BS; i++) {
    ...
    if (is_mapped && is_owned_by_pid && is_in_range) {
      *pageth = virtual_page_number - bsm_tab[i].bs_vpno; // Calculate page offset
      *store = i;  // Assign backing store index
